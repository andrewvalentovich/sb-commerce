...
const current = ref<models.Product>(null)

watch(() => dialog.dialogs.catalogProduct.active, v => {
    if (v) {
        bodyFixed()
        showModal()
    } else {
...
<template>
    <!-- authorization Popup -->
    <DialogModal max-width="1100px" :centered="true" :noFooter="false" :show="dialog.dialogs.catalogProduct.active" @close="closeModal()">
        <template #content>
