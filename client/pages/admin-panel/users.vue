...
const userFilter = new Filter(adminUserStore, 'filterParams')
await userFilter.parseUrlQuery()

watch(adminUserStore.filterParams, v => {
    userFilter.updateUrlQuery(v, 'admin-panel-users')
    adminUserStore.get(userFilter.getCloneFilterParams())
}, {
    deep: true
})
...
async function destroy(item: models.User) {
    ...
    await adminUserStore.get(userFilter.getCloneFilterParams());
}
...
<template>
                ...
                <div class="flex items-center mb-4 sm:mb-0">
                    <div class="sm:pr-3">
                        ...
                        <div class="relative w-48 mt-1 sm:w-64 xl:w-96">
                            <input
                                v-model="adminUserStore.filterParams.search"
                                type="text"
                                name="email"
                                id="users-search"
            ...
            <div class="inline-block min-w-full align-middle">
                <div class="overflow-hidden shadow">
                    <table class="min-w-full divide-y divide-gray-200 table-fixed dark:divide-gray-600">
                        ...
                        <tbody v-if="!(adminUserStore.loading || enumStore.loading)" class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                    ...
                    <span class="p-3" v-if="adminUserStore.loading || enumStore.loading">Загрузка...</span>
                    <span class="p-3" v-if="adminUserStore.items.length == 0 && (!adminUserStore.loading && !enumStore.loading)">Нет пользователей</span>
                </div>
            </div>
        </div>
    </div>
    <div class="sticky bottom-0 right-0 items-center w-full p-4 bg-white border-t border-gray-200 sm:flex sm:justify-between dark:bg-gray-800 dark:border-gray-700">
        <TailwindPagination
            ...
            @pagination-change-page="adminUserStore.changePage"
        />
    </div>
